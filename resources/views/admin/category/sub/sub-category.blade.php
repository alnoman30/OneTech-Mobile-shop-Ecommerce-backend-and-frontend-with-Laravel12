                                ...
                                <div class="item-title">
                                    <div class="col-xs-6">
                                        <a href="{{ route('admin.subcategory.add')}}"
                                            class="btn btn-md btn-info">Add Sub-Category</a>
                                    </div>
                                </div>
                                        ...
                                        <tbody>
                                            @foreach ($subcategories as $scategory)
                                                <tr role="row" class="odd">
                                                    <td>{{ $scategory->category->name ?? 'N/A' }}</td> <!-- main category name -->
                                                    <td class="sorting_1">{{ $scategory->name }}</td>
                                                    <td>{{ $scategory->slug }}</td>
                                                    <td>{{ $scategory->description }}</td>
                                                    <td>
                                                        @if ($scategory->status === 'active')
                                                    ...
                                                    <td>
                                                        <div class="action__buttons">
                                                            <a href="{{ route('admin.scategory.edit', $scategory->id)}}" class="btn-action" title="Edit"><i class="fas fa-pen-to-square"></i></a>
                                                            <a href="javascript:void(0);" class="btn-action deleteBtn" 
                                                                data-id="{{ $scategory->id }}" data-bs-toggle="modal" data-bs-target="#deleteModal" 
                                                                title="Delete">
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        deleteButtons.forEach(button => {
            button.addEventListener('click', function () {
                ...
                const route = "{{ route('admin.scategory.delete', ':id') }}".replace(':id', categoryId);
